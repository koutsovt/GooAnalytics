...
- Total interactions: ${data.local.totalInteractions} (${data.local.interactionsDelta > 0 ? "+" : ""}${data.local.interactionsDelta}% vs prior month)`
    : data.connections.ga4
      ? `LOCAL PRESENCE (Google Business Profile)
- NOT MEASURED: the Business Profile is not linked to Google Analytics, so off-site customer actions (calls, directions, bookings) are not tracked. Treat these as unmeasured, NEVER as zero. Setting up this link so bookings can be measured is a high-value action.`
      : `LOCAL PRESENCE (Google Business Profile)
...
- Conversion blind spot: if customer actions (calls, directions, bookings) are NOT MEASURED or NOT CONNECTED, the single most valuable action is to set up that tracking — you cannot improve what you cannot see. Say so plainly instead of staying silent on it.
...
- Put the action that unlocks measurement or the biggest revenue first. Do not pad with low-impact busywork just to reach three.
