 ...
 *   { "currency": "AUD", "services": [{ "name": "...", "price": 35, "raw": "from $35" }] }
 */
export function buildCompetitorExtractPrompt(
  pageText: string,
  businessName: string,
  countryHint?: string,
): string {
  ...
  const countryHintLine = countryHint
    ? `\nThis business is based in ${countryHint} (per its schedule timezone); prefer the local currency for a bare $ sign unless the page clearly indicates otherwise.\n`
    : "";
...
- Infer "currency" as an ISO code (GBP, EUR, USD, AUD…) from the currency symbol or context — £ means GBP, € means EUR, and a distinctly-labelled $ (e.g. "US$", "NZ$") means that market's dollar. A bare, unlabelled $ is ambiguous, so default to "AUD" for it unless the page or business context points elsewhere. If there are no prices, still return your best-guess currency.
