...
export function AuditLogsViewer({ onLoad }: AuditLogsViewerProps) {
  ...
  if (logs.length === 0) {
    return <div className="text-muted-foreground text-sm">No audit logs yet.</div>;
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border">
            ...
            <th className="text-left px-4 py-2 font-semibold text-foreground">Target</th>
            <th className="text-left px-4 py-2 font-semibold text-foreground">Date</th>
          </tr>
        </thead>
        <tbody>
          {logs.map((log) => (
            <tr key={log.id} className="border-b border-border hover:bg-surface">
              <td className="px-4 py-3 text-foreground">{getActionLabel(log.action)}</td>
